    ...
    <section>
      <div class="drop" id="drop">
        ...
        <div class="drop-empty" id="empty">
          <strong>Toque ou cole a foto</strong>
          <span>PNG · JPG · WEBP · Ctrl+V</span>
        </div>
...
function takeImageFromClipboard(items){
  if(!items) return false;
  for(var i=0;i<items.length;i++){
    var item=items[i];
    if(item.kind==="file" && item.type && item.type.indexOf("image/")===0){
      var pasted=item.getAsFile();
      if(pasted){
        var ext=(pasted.type.split("/")[1])||"png";
        setFile(new File([pasted], pasted.name||("colar."+ext), {type:pasted.type}));
        return true;
      }
    }
  }
...
window.addEventListener("paste", function(e){
  ...
  if(takeImageFromClipboard(e.clipboardData && e.clipboardData.items)){
    e.preventDefault();
    toast("Imagem colada.");
  }
});
